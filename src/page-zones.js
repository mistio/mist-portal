...
export default class PageZones extends connect(store)(LitElement) {
  static get styles() {
    return css`
      :host {
  ...
  constructor() {
    ...
    this.actions = [
      ...
      },
      {
        ...
        theme: 'primary',
        icon: html``,
        run: () => () => Router.go(`/portal/orgs/${this.orgName}/zones/+add`),
        condition: items => !items.length,
      },
    ];
  }
  ...
  render() {
    return html` <mist-list
      ...
      @active-item-changed=${e => {
        if (e.detail.value)
          Router.go(`/portal/orgs/${this.orgName}/zones/${e.detail.value.id}`);
      }}
    >
    </mist-list>`;
  }
