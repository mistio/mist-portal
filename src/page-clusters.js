...
export default class PageClusters extends connect(store)(LitElement) {
  static get styles() {
    return css`
      :host {
  ...
  constructor() {
    ...
    this.actions = [
      ...
      },
      {
        ...
        icon: html``,
        run: () => () =>
          Router.go(`/portal/orgs/${this.orgName}/clusters/+create`),
        condition: items => !items.length,
      },
    ];
  }
  ...
  render() {
    return html` <mist-list
      ...
      @active-item-changed=${e => {
        if (e.detail.value)
          Router.go(
            `/portal/orgs/${this.orgName}/clusters/${e.detail.value.id}`
          );
      }}
    >
    </mist-list>`;
